...
class cHome:
    ...
    def load(self):
        ...
        if (cConfig().getSetting('home_cherches') == 'true'):
            ...
            oGui.addDir(SITE_IDENTIFIER, 'showSearch', '[COLOR lightgray]' +cConfig().getlanguage(30076)+ '[/COLOR]', 'search.png', oOutputParameterHandler)
            
        if (cConfig().getSetting('home_cherchev') == 'true'):
            oOutputParameterHandler = cOutputParameterHandler()
            oOutputParameterHandler.addParameter('siteUrl', 'http://venom')
            oGui.addDir('themoviedb_org', 'load','[COLOR lightgray]'+ cConfig().getlanguage(30088)+'[/COLOR]', 'searchtmdb.png', oOutputParameterHandler)
        ...
        if (cConfig().getSetting('home_videos') == 'true'):
            oOutputParameterHandler = cOutputParameterHandler()
            oOutputParameterHandler.addParameter('siteUrl', 'http://venom')
            oGui.addDir(SITE_IDENTIFIER, 'showNets', cConfig().getlanguage(30114), 'buzz.png',
oOutputParameterHandler)
        ...
        oOutputParameterHandler = cOutputParameterHandler()
        oOutputParameterHandler.addParameter('siteUrl', 'http://venom')
        oGui.addDir('cDownload', 'getDownload','[COLOR teal]'+cConfig().getlanguage(30202)+'[/COLOR]', 'download.png', oOutputParameterHandler)
